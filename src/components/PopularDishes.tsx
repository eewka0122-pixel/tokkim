      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {promos.map((promo) => (
            <div 
              key={promo.id} 
              className="relative group overflow-hidden rounded-[2rem] shadow-lg hover:shadow-2xl transition-all duration-500 h-[380px] md:h-[420px] cursor-pointer"
            >
              {/* Фоновая картинка */}
              ...
              {/* Текст снизу (описание выезжает при наведении) */}
              <div className="absolute bottom-0 left-0 w-full p-8 translate-y-8 group-hover:translate-y-0 transition-transform duration-500 ease-out z-20">
                <h3 className="font-serif text-2xl font-bold text-white mb-3 drop-shadow-md">
                  {promo.title}
                </h3>
                <p className="text-white/90 text-sm leading-relaxed opacity-0 group-hover:opacity-100 transition-opacity duration-500 delay-75">
                  {promo.description}
                </p>
              </div>
            </div>
